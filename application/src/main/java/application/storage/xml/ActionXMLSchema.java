...
public class ActionXMLSchema implements XMLSchema<Action> {
	...
	public Action parse(Element root) throws DataConversionException {
		...
		final List<Element> elements = root.getChildren(actionFunctionXMLSchema.getSchemaName());
		for (Element element : elements) {
			// TODO (2021-12-15 #50): добавить значения functionType в xsd-схему
			final ActionFunction actionFunction = actionFunctionXMLSchema.parse(element);
			final String functionType = element.getAttributeValue(TAG_functionType);
	...
	public Element combine(Action action) {
		final Element root = new Element(TAG_action);
		...
		for (ActionFunction preConditionChecker : action.getPreConditionCheckers()) {
			final Element element = actionFunctionXMLSchema.combine(preConditionChecker);
		...
		for (ActionFunction parameterUpdater : action.getParameterUpdaters()) {
			final Element element = actionFunctionXMLSchema.combine(parameterUpdater);
