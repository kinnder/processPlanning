...
public class SystemOWLSchema implements OWLSchema<System> {
	...
	public Individual combine(System system) {
		final Individual ind_system = owlModel.newIndividual_System();
		ind_system.addLabel("System", "en");
		ind_system.addLabel("Система", "ru");

		final Map<String, Individual> ind_systemObjects = new HashMap<String, Individual>();

		for (SystemObject systemObject : system.getObjects()) {
			final Individual ind_systemObject = systemObjectOWLSchema.combine(systemObject);
		...
		for (Link link : system.getLinks()) {
			final Individual ind_link = linkOWLSchema.combine(link);
	...
	public System parse(Individual ind_system) {
		...
		owlModel.getClass_SystemObject().listInstances().filterKeep((ind_object) -> {
			...
			final SystemObject systemObject = systemObjectOWLSchema.parse(ind_object.asIndividual());
		...
		owlModel.getClass_Link().listInstances().filterKeep((ind_link) -> {
			...
			final Link link = linkOWLSchema.parse(ind_link.asIndividual());
			system.addLink(link);
		});
