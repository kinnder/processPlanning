...
public class PreConditionCheckerOWLSchemaTest {
	...
	PreConditionCheckerOWLSchema testable;
	...
	public void setup() {
		...
		testable = new PreConditionCheckerOWLSchema(owlModel_mock, luaScriptLineOWLSchema_mock);
	}

	@Test
	public void newInstance() {
		testable = new PreConditionCheckerOWLSchema(new SystemTransformationsOWLModel());
	}
	...
	public void combine() {
		...
		final LuaScriptActionPreConditionChecker preConditionChecker = new LuaScriptActionPreConditionChecker(globals,
				scriptLines);
		final Individual i_preConditionChecker = context.mock(Individual.class, "i-preConditionChecker");
		...
		context.checking(new Expectations() {
			{
				oneOf(owlModel_mock).newIndividual_PreConditionChecker();
				will(returnValue(i_preConditionChecker));
				...
				oneOf(i_line_mock).addProperty(op_isLineOf_mock, i_preConditionChecker);

				oneOf(owlModel_mock).getObjectProperty_hasLine();
				will(returnValue(op_hasLine_mock));

				oneOf(i_preConditionChecker).addProperty(op_hasLine_mock, i_line_mock);
			}
		});

		assertEquals(i_preConditionChecker, testable.combine(preConditionChecker));
	}
	...
	public void parse() {
		...
		context.checking(new Expectations() {
			{
				...
				oneOf(i_preConditionChecker).hasProperty(op_hasLine_mock, i_line_mock);
		...
		LuaScriptActionPreConditionChecker result = (LuaScriptActionPreConditionChecker) testable
				.parse(i_preConditionChecker);
